...
class DisplayService {
 ...
 private:
  ...
  bool lastRenderedWasMetric_ = false;
  bool lastMetricStaleWarning_ = false;
  ...
  void renderMetric(MetricId metric, const ExternalValues& values, bool staleWarning);
  void renderClock(const struct tm& now);
  void renderConnecting();
  void renderFrame(const DisplayFrame& frame, bool staleWarning);
  void renderError();
